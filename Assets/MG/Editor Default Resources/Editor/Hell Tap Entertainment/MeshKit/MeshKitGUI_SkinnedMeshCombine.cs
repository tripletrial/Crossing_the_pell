...
namespace HellTap.MeshKit {
	...
	/// -> GET CURRENT COMBINE BUTTON NAME
		...
		public static string GetCurrentCombineButtonName( MeshKitGUI mkGUI ){
			...
			} else if( mkGUI.combineSelGridInt == 1 && mkGUI.shouldCreateNewMeshKitCombineMeshWithAtlas == true ){
	...
	/// -> DRAW OPTIONS
		...
		public static void DrawOptions( MeshKitGUI mkGUI ){
			...
			if( mkGUI.GetHowManyMeshKitAutoLODComponentsInChildren() > 0 || 
				...
				HTGUI.WrappedTextLabel("<size=10><b>NOTE:</b> You cannot combine objects that contain or are a part of an LOD system.</size>" );
			...
			if( mkcsm == null ){
				...
				HTGUI.WrappedTextLabel("<size=10>Press the Setup Combine button to begin the process...</size>" );
			...
			// ---------------------
				...
				// ---------------------------------
					...
					} /*else if( mkcsm.dropDownHelperList == MeshKitCombineSkinnedMeshSetup.DropDownHelperList.StandardSpecularShader ){
						mkcsm.propertyList = mkcsm._defaultStandardSpecularPropertyList;
						mkcsm.dropDownHelperList = MeshKitCombineSkinnedMeshSetup.DropDownHelperList.Select;
					...
					*/
					...
					mkcsm.useAdvancedPixelProcessing = HTGUI_UNDO.ToggleField( mkcsm, "Use Advanced Pixel Processing", MeshKitGUI.gearIcon, "Use Advanced Pixel Processing", mkcsm.useAdvancedPixelProcessing  ); 

					// If we've enabled Advanced Pixel Processing, handle it
					if( mkcsm.useAdvancedPixelProcessing == true ){

						int result = 0;

						// These classes have the GUI Layout built in! They return 0 if no issues occured.
						result = BakeFloatIntoTextureProperty.DoGUILayout( mkcsm, ref mkcsm.bakeFloatsIntoTextures, MeshKitGUI.goIcon, HTGUI.removeButton, HTGUI.addButton );
						if( result == 1 ){
						...
						// These classes have the GUI Layout built in! They return 0 if no issues occured.
						result = BakeColorIntoTextureProperty.DoGUILayout( mkcsm, ref mkcsm.bakeColorIntoTextures, MeshKitGUI.goIcon, HTGUI.removeButton, HTGUI.addButton );
						if( result == 1 ){
				...
				// Use "Combine Meshes With Texture Atlasing" Mode
					...
					if( AssetDatabase.IsValidFolder( mkcsm.assetRelativePathToFolder ) == true ){
						...
						combineWasSuccessful = EditorSkinnedMeshRendererUtility.CombineSkinnedMeshRenderers( 
							...
							mkcsm.useAdvancedPixelProcessing ? mkcsm.bakeFloatsIntoTextures : null,		// <- Experimental
							mkcsm.useAdvancedPixelProcessing ? mkcsm.bakeColorIntoTextures : null		// <- Experimental
						);
